...
//Gap detection
MATCH (cs:CustomerSegment) WHERE NOT (cs)-[:HAS_STAKEHOLDER]->() RETURN cs.name AS entity, 'No stakeholders' AS gap UNION ALL MATCH (pm:PricingModel) WHERE NOT (pm)-[:HAS_ASSUMPTION]->()RETURN pm.type AS entity, 'No pricing assumptions' AS gap UNION ALL MATCH (sl:SustainabilityLayer) WHERE NOT (sl)<-[:RELATES_TO_SUSTAINABILITY]-(:Assumption) RETURN sl.type AS entity, 'No sustainability assumption linked' AS gap
